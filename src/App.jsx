...
export default function App() {
  ...
  useEffect(()=> {
    document.getElementById("codeOutput").value = `${name.replace("-", "").replace(".", "").replace(",", "").replace("/", "").replace("\\", "").replace("\"", "").replace("\'", "").replace(" ", "")}JSON =  ${JSON.stringify(targets)}
        self.auton_selector.setDefaultOption("${name}", autoRunner(self.s_Swerve, self.s_Intake, self.s_Indexer, self.s_Shooter, self.s_Arm, self.s_Vision, ${name.replace("-", "").replace(".", "").replace(",", "").replace("/", "").replace("\\", "").replace("\"", "").replace("\'", "").replace(" ", "")}JSON).getCommand())`
    //console.log(targets)
